...
use case is "I want to download the Euclid image of my favourite object". In order to do this, one must know the coordinates of the selected object as well as the type of image (e.g. calibrated, mosaic, ...) and instrument that was used to acquire the image (e.g., VIS)
that is going to be downloaded. In the following example we proceed in three steps: first query the "calibrated_frame" TAP_ table to find out which FITS files contain an observation that covers the sky region where our selected target (NGC 6505; [RA, Dec] = [267.7808, 65.5308]) is located. 
Then, we use the get_product_ method to download one of the associated VIS calibrated images (the Euclid product: DpdVisCalibratedQuadFrame_). The image is finally displayed using Matplotlib.
...
**Step 1:** Launch an ADQL_ query to retrieve the file_name of the FITS files that contain VIS observations of an sky region where NGC 6505 is located. This information is stored in the "calibrated_frame" TAP_ table, that includes a "fov" field with that indicates
the field of view of the individual observations. Note that to ensure that the target location is not too close to the image edges, a radius of half an arc-minute is used inside of the INTERSECTS(CIRCLE) clause. 

  >>> radius   = 0.5/60.  # Use a radius of half arcminute to ensure that the target is not located near the image edges.
  >>> query    = f"SELECT file_name, file_path, datalabs_path, frame_seq, instrument_name, filter_name, ra, dec, creation_date, duration FROM calibrated_frame \
                 WHERE instrument_name='VIS' AND fov IS NOT NULL AND INTERSECTS(CIRCLE(267.7808, 65.5308, {radius}), fov)=1)"
