...
(but note that all the queries time out after 7200 seconds). The example below launches a cone search and it applies a filter on the TbD 
reduced version of the first ADQL_ query
example listed in the `Euclid Archive <https://eas.esac.esa.int/sas/>`_ (see its "Search/ADQL FORM" subtab).
