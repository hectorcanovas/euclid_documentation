...
that is going to be downloaded. In the following example we first query the "calibrated_frame" TAP_ table to find out which FITS files contain an observation that covers the sky region where our selected target (NGC 6505; [RA, Dec] = [267.7808, 65.5308]) is located. 
Then, we use the get_product_ method to download one of the associated VIS calibrated images (the Euclid product: DpdVisCalibratedQuadFrame_). The image is then displayed using Matplotlib.
